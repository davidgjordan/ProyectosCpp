...
class IComparable : public virtual IInterface{//todas son clases abstractas
public:
...
class String : public virtual Object, public virtual IComparable{/*la ultima de la jerarquia decimos*/
...
public:
    ...
    void rehash(){
        ...
        for(auto i=0; i<cap;i++){
            ...
            auto aux = slot->first;
            while(aux!= nullptr){//mientras eas diferente de null lo anado al otro array
                auto next = aux->next;
                aux->next = nullptr;
...
public:
    string toString() const override{
        ...
        for(auto i=0;i<cap;i++){
            auto slot = data[i];
            if(slot == nullptr)continue;//si el elemento en esta pocicion es null continuo
    ...
    Object * operator[](const Object & src)const{//el q use la funcion elije q acer con el objeto
        auto hc = src.getHashCode();

        auto pos = hc%cap;
        auto slot = data[pos];
        if(slot == nullptr)return nullptr;//si el slot es null no existe si no
