...
NeuralNetwork::NeuralNetwork(int inputNodes,int quantityLayersHidden,int hiddenNodes,int outputNodes)// : NeuralNetworkDTO(inputNodes, quantityLayersHidden, hiddenNodes, outputNodes)
{
}
NeuralNetwork::NeuralNetwork(int inputNodes,int quantityLayersHidden,int hiddenNodes,TypeOfTransition hiddenFunction,int outputNodes,TypeOfTransition outputFunction,double learnRate,double momentum,double error) : NeuralNetworkDTO(inputNodes, quantityLayersHidden, hiddenNodes, outputNodes,learnRate, momentum, error)
{
	int inputsPerNode = 1;
	int weights = inputNodes;
	neuralNetwork.push_back(new Layer(inputNodes,LayerType::INPUT, distribution, generator));
...
void NeuralNetwork::Train(vector<vector<double>>& inputs, vector<vector<double>>& expectedValue)
{
	Normalization norm;
...
vector<double> NeuralNetwork::Evaluate(vector<double>& inputt)
{

	vector<vector<double>> normObs;
	normObs.push_back(inputt);
	...
	vector<double> input = norm.Normalize_WhitDates(normObs);
