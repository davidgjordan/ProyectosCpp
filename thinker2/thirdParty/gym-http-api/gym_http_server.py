...
class Envs(object):
    ...
    def step(self, instance_id, action, render):
        ...
        renderRGB = env.render('rgb_array')
        [observation, reward, done, info] = env.step(nice_action)
        obs_jsonable = env.observation_space.to_jsonable(observation)
        return [obs_jsonable, reward, done, info, json.dumps(renderRGB.tolist())]
...
def env_step(instance_id):
    ...
    BoolRender = get_optional_param(json, 'render', False)
    [obs_jsonable, reward, done, info, render] = envs.step(instance_id, action, BoolRender)
    return jsonify(observation = obs_jsonable,
                    reward = reward, done = done, info = info, render=render)
